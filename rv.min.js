...
//        PlanetVy = Math.sqrt(mtot*(1+ecc)/(semia*(1-ecc)));
  ...
  totaltime = 0;
  count = 0;
  timeResolution = Math.pow(semia,1.5)/200.;
  clearChart();
  updateChart();
...
function setScale() {
  ...
  for (var i=0; i<auBins.length; i++) {
    if (semia <= auBins[i] ) {
      AuPerPixel = 1.2*2*auBins[i]/theSunCanvas.width;
      break;
    }
  }
}
...
function drawSun() {
  ...
  //
  // theGradient.addColorStop(0, "#D7DF01"); // "#ffd0d0"  "#ff0000"
  // theGradient.addColorStop(1, "#F2F5A9");
...
function showNewVars() {
  ...
  mReadout.innerHTML = "10^" + mSlider.value;
}
